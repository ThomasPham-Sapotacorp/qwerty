...
const folderUpload 	= 'public/backend/images/' + type + '/';

const BannerModel 		= require(__path_models + type);
const BannerValidate 	= require(__path_validates + type);

const FilterStatusHelper 	= require(__path_helpers + 'filter-status');
const ParamsHelper 			= require(__path_helpers + 'params');
const FilesHelper 			= require(__path_helpers + 'files');
const UploadMiddleware		= require(__path_middleware + 'uploadBanner');
...
router.get('/', async(req, res, next) => {
	...
	await BannerModel.countItem(params.check).then((data) => {
		params.pagination.totalItems = data;
	});

	BannerModel.listItem(params).then((items) => {
...
router.get('/change-status/:id/:status', (req, res, next) => {
	...
	let user			= req.user;
	BannerModel.changeStatus(id, user, currentStatus, {task: "update-one"}).then((result) => {
...
router.post('/change-status/:status', (req, res, next) => {
	...
	let id 	= req.body.id;
	let user = req.user;
	BannerModel.changeStatus(id, user, currentStatus, {task: "update-multi"}).then((result) => {
...
router.get('/delete/:id/', (req, res, next) => {
	let id = ParamsHelper.getParam(req.params, 'id', '');
	BannerModel.deleteItem(id, {task: "delete-one"}).then((result) => {
...
router.post('/delete/', (req, res, next) => {
	let id = req.body.id;
	BannerModel.deleteItem(id, {task: "delete-multi"}).then((result) => {
...
router.post('/change-ordering/', (req, res, next) => {
	...
	let user		= req.user;
	BannerModel.changeOrdering(orderings, ids, user).then((result) => {
...
router.get(('/add(/:id)?'), async (req, res, next) => {
	...
	if(id === ''){
		...
		BannerModel.getItem(id).then((item) => {
...
//Save
router.post(('/save'), UploadMiddleware,  BannerValidate, async (req, res, next) => {
	...
	if(errorUpload) {
		errors.push({param: 'image', msg: errorUpload});
	}	
	...
	if(errors.length){
		...
		if(req.file){
			...
			item.image = item.imageOld;
		}
		
		BannerModel.saveItem(item, user, {task: taskCurrent}).then((result) => {
